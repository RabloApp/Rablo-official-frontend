import Image from 'next/image'
import React from 'react'

const Benefits = () => {
  return (
    <div className='w-full h-[41.46vw] bg-[#5271FF80] flex flex-col justify-center items-center gap-[3.5vw] relative'>

      {/* Decorative background shapes at bottom left */}
      <div className='absolute bottom-[-2vw] left-[3vw] flex flex-col z-0'>
        <Image src="/mainSection/images/circle.svg" width={160} height={160} alt='circle' className='mb-[-1vw]' />
        <Image className='mt-[-1.33vw] ml-[-1.56vw]' src="/mainSection/images/shapes.svg" width={100} height={100} alt='shapes' />
      </div>

      {/* Title */}
      <h1 className='font-bold text-[2.6vw] border-b-2 z-10'>Benefits of choosing Rablo</h1>

      {/* Boxes Container */}
      <div className='w-[87.5vw] h-[21.46vw] flex justify-between gap-[4.4vw] z-10'>

        {/* Box 1 */}
        <div className='w-[41.67vw] h-[21.46vw] rounded-[0.83vw] bg-white pt-[2.08vw] pb-[2.08vw] pl-[1.25vw] pr-[1.25vw] flex flex-col gap-[1.25vw] ml-[20px]'>
          <h2 className='font-bold text-[1.7vw]'>For Tutors</h2>
          <div>
            <div className='flex items-center gap-[0.9vw] p-[0.5vw]'>
              <Image src="/mainSection/images/Icon1.svg" width={48} height={48} alt='icon' />
              <p className='text-[1.25vw] font-medium'>Zero Commission</p>
            </div>
            <div className='flex items-center gap-[0.9vw] p-[0.5vw]'>
              <Image src="/mainSection/images/Icon2.svg" width={48} height={48} alt='icon' />
              <p className='text-[1.25vw] font-medium'>Exposure to local and online students</p>
            </div>
            <div className='flex items-center gap-[0.9vw] p-[0.5vw]'>
              <Image src="/mainSection/images/Icon3.svg" width={48} height={48} alt='icon' />
              <p className='text-[1.25vw] font-medium'>Easy Payouts</p>
            </div>
          </div>
        </div>

        {/* Box 2 */}
        <div className='w-[41.67vw] h-[21.46vw] rounded-[0.83vw] bg-white pt-[2.08vw] pb-[2.08vw] pl-[1.25vw] pr-[1.25vw] flex flex-col gap-[1.25vw]'>
          <h2 className='font-bold text-[1.7vw]'>For Students</h2>
          <div>
            <div className='flex items-center gap-[0.9vw] p-[0.5vw]'>
              <Image src="/mainSection/images/Icon4.svg" width={48} height={48} alt='icon' />
              <p className='text-[1.25vw] font-medium'>Verified Tutors</p>
            </div>
            <div className='flex items-center gap-[0.9vw] p-[0.5vw]'>
              <Image src="/mainSection/images/Icon5.svg" width={48} height={48} alt='icon' />
              <p className='text-[1.25vw] font-medium'>Flexible Scheduling</p>
            </div>
            <div className='flex items-center gap-[0.9vw] p-[0.5vw]'>
              <Image src="/mainSection/images/Icon6.svg" width={48} height={48} alt='icon' />
              <p className='text-[1.25vw] font-medium'>One on one support</p>
            </div>
          </div>
        </div>

      </div>
    </div>
  )
}

export default Benefits








